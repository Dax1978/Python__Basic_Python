...
class Scaner(OfficeEquipment):
    ...
    @classmethod
    def add(cls):
        brend = input("Укажите бренд сканера: ")
        model = input("Укажите модель сканера: ")
        paper = cls.int_value(
            "Укажите размер сканируемого листа. 1 - А4, 2 - А3, 3 - А2, 4 - А1, 5 - А0: ", 1, 5)
        roll_flat = cls.int_value(
            "Укажите 1 или 2, для протяжного или планшетного сканера соответственно: ", 1, 2)
        add_from = input("Укажите откуда прибыл сканер на склад: ")
        add_date = datetime.datetime.now().strftime("%d-%m-%Y %H:%M")
        el = Scaner(brend, model, paper, roll_flat)
